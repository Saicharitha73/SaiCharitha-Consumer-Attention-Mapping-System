...
// Register a new user (Manager or Worker) and save to user registry file data
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      const existingEmpId = users.find(
        (u) => u.employee_id.trim().toLowerCase() === userData.employeeId.trim().toLowerCase()
      );
      if (existingEmpId) {
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (user.role !== expectedRole) {
        return reject(
          new Error(
            `Access denied. This account is registered as a ${user.role}, not a ${expectedRole}.`
          )
        );
      }
...
export const authenticateUserOTP = async (target, code, expectedRole) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (user.role !== expectedRole) {
        return reject(new Error(`Access denied. Account is registered as ${user.role}, not ${expectedRole}.`));
      }
